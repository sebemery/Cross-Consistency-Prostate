...
class BaseTrainerMulti:

    def __init__(self, model, resume, config, iters_per_epoch, site, train_logger=None):
        ...
        # SETTING THE DEVICE
        self.device, availble_gpus, self.str_device = self._get_available_devices(self.config['n_gpu'])
        ...
        run_name = config['experim_name'] + '_' + site
        ...
        self.html_results = HTML(web_dir=config['trainer']['save_dir'], exp_name=run_name,
                                 save_name=run_name, config=config, resume=resume)
    ...
    def _get_available_devices(self, n_gpu):
        ...
        str_device = 'gpu' if n_gpu > 0 else 'cpu'
        self.logger.info(f'Detected GPUs: {sys_gpu} Requested: {n_gpu}')
        available_gpus = list(range(n_gpu))
        return device, available_gpus, str_device
    ...
    def train(self, epoch):
        ...
        if epoch % self.save_period == 0:
            self._save_checkpoint(epoch, self.site, save_best=self.improved)
    ...
    def validation(self, epoch):
        ...
        if self.mnt_mode != 'off' and epoch % self.config['trainer']['val_per_epochs'] == 0:
            try:
                ...
                self.logger.warning('The metrics being tracked ({self.mnt_metric}) has not been calculated. Training stops.')
    ...
    def _save_checkpoint(self, epoch, site, save_best=False):
        state = {
        ...
        filename = os.path.join(self.checkpoint_dir, f'checkpoint_{site}.pth')
        self.logger.info(f'\nSaving a checkpoint: {filename} ...')
        torch.save(state, filename)

        if save_best:
            filename = os.path.join(self.checkpoint_dir, f'best_model_{site}.pth')
            torch.save(state, filename)
            self.logger.info("Saving current best: best_model_"+site+".pth")
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.logger.info(f'Checkpoint <{resume_path}> (epoch {start_epoch}) was loaded')
