...
from utils.helpers import DeNormalize, scale
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx in tbar:
            ...
            if batch_idx % int(len(self.unsupervised_loader)) == 0:
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(tbar):
                ...
                mIoU = IoU.mean()
                mdice = dice.mean()
                seg_metrics = {"Pixel_Accuracy": np.round(pixAcc, 3), "Mean_IoU": np.round(mIoU, 3),"Mean_dice": np.round(mdice, 3),"Class_IoU": dict(zip(range(self.num_classes), np.round(IoU, 3))),"Class_dice": dict(zip(range(self.num_classes), np.round(dice, 3)))}

                tbar.set_description('EVAL ({}) | Loss: {:.3f}, PixelAcc: {:.2f}, Mean IoU: {:.2f} Mean Dice {:.2f} |'.format(epoch, total_loss_val.average, pixAcc, mIoU, mdice))
